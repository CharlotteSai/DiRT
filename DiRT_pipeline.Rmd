...
Before start, the genome file might needed. This information can be pull out from the target dataset `bam` file header.
...
closest_tRNA_gene <- rbind(closest_tRNA_gene.up,closest_tRNA_gene.down) %>% 
  ...
  mutate(tRNAFirst = tRNA.start < gene.start,
         ...
         interval.length = ifelse(tRNAFirst == TRUE, 
                                  (gene.start-1)-(tRNA.end+1)+1, (tRNA.start-1)-(gene.end+1)+1)) #1104
```

## subset for proper distance (not overlapped) for tRNA-gene combination and add unique id to them
This pipeline is designed for non-stranded library, so non-overlapped tRNA-gene combinations are the proper input. tRNA or gene id stand itself may cause issue for one match many, therefore, unique combination id is needed.
...
```{r}
seq.size <- "./Essential_files/Vv_genome.txt"
# this should be consistant with the target bam dataset 
...
```{r}
# specify Samples
bam.dir <- "~/biohub/160830_RNAseq_terroir/2_HISAT2" # need change based on where I use RSudio
...
# differdent str_extract pattern can be applied according to your bam names
...
```{r}
# filter expressed tRNA in all 6 vineyards
tRNA.expressed.AllSample <- rowSums(dplyr::select(tRNA.counts,6:11) >= 1) == 6 
# return logical value for all read count col
sum(tRNA.expressed.AllSample) #117
...
```{r}
# filter expressed tRNA in all 6 vineyards
gene.expressed.AllSample <- rowSums(dplyr::select(gene.counts, 6:11) >= 1) == 6
sum(gene.expressed.AllSample) #102-1
common.gene <- gene.counts[gene.expressed.AllSample,]
tRNA.gene.expressed.combo <- filter(tRNA.expressed.gene.combo, gene.id %in% common.gene$gene.id) #102-1 gene
```
...
### use ``coverage()`` to look through ``bam`` files for coverage of each base
...
tRnaToGene <- aveCoverage.final %>%
  ...
  split(f = .$id) %>%
  lapply(function(x){
    if (nrow(x) == 1) return(x) # This removes the genes with >1 tRNA, the issue about gene with muiltiple tRNA is fixed after I give tRNA_gene combo id  
  }) %>%
  bind_rows() 
#127 in total (less gene than combind bamfile may because we filter the expression for commonly expressed tRNA and gene rather than just start with 232)
```
...
```{r}
allIntergenic <- names(coverage.intergenic$Sample_20)
